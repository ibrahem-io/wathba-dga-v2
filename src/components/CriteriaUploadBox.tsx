// src/components/CriteriaUploadBox.tsx - Simplified version using Assistants API
...
export default function CriteriaUploadBox({
  ...
  const handleFiles = (files: File[]) => {
    const maxSize = 50 * 1024 * 1024; // 50MB (OpenAI limit)
    const allowedTypes = [
      'application/pdf', 
    ...
    const validFiles = files.filter(file => {
      ...
      if (file.size > maxSize) {
        setError(language === 'ar' ? 'حجم الملف يجب أن يكون أقل من 50 ميجابايت' : 'File size must be less than 50MB');
        return false;
      }
      if (!allowedTypes.includes(file.type)) {
        setError(language === 'ar' 
          ? 'يرجى رفع ملفات PDF أو DOCX أو TXT فقط' 
  ...
  const analyzeFiles = async (files: File[]) => {
    ...
    console.log(`🚀 Starting OpenAI analysis for ${files.length} files`);
    setIsAnalyzing(true);
    setError('');

    try {
      // Use OpenAI Assistants API to handle file processing and analysis
      const result = await analyzeDocumentForCriteria(files, criteriaId, language);
      ...
      {uploadedFiles.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs text-gray-500 mb-3">
            {language === 'ar' 
              ? 'ملفات PDF أو DOCX أو TXT (حد أقصى 50 ميجابايت لكل ملف)'
              : 'PDF, DOCX, or TXT files (Max 50MB each)'}
          </p>
          <p className="text-xs text-green-600 mb-3 font-medium">
            {language === 'ar' 
              ? '🤖 يتم التحليل مباشرة بواسطة OpenAI - قراءة مثالية للملفات!'
              : '🤖 Analyzed directly by OpenAI - Perfect file reading!'}
          </p>
          <input
            type="file"
            accept=".pdf,.docx,.txt,.csv,.md,.doc"
      ...
      {isAnalyzing && (
        <div className={`mt-4 p-3 bg-blue-50 rounded-lg flex items-center space-x-2 ${language === 'ar' ? 'space-x-reverse' : ''}`}>
          ...
          <span className="text-blue-700 text-sm">
            {language === 'ar' ? 'جاري تحليل الملفات مباشرة بواسطة OpenAI...' : 'Analyzing files directly with OpenAI...'}
          </span>
        </div>
      )}
